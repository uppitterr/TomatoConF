...
Процедура СозданиеИЗаполнениеДокументов(Данные)
	...
	для каждого Элемент из Данные цикл
		ПараметрыОткрытия = Новый Структура ("ЗначенияЗаполнения", Элемент);
	   	ОткрытьФорму("Документ.сжсвПереводИзГруппыВГруппу.ФормаОбъекта",ПараметрыОткрытия,,новый УникальныйИдентификатор);
	КонецЦикла;
...
Функция ПрочитатьДанныеДляВводаНаОсновании(ДокументСсылка)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	сжсвЖивотныеВНаличииОстатки.ГруппаЖивотных КАК Группа,
	               ...
	               |	Местоположение.Группа,
	               |	Местоположение.ТехнолГруппа,
	...
	для каждого Строка из ТабРазрезыУчета цикл
		...
		РезультатСтр.Вставить("Группа", Строка.Группа);
		РезультатСтр.Вставить("ТехнолГруппа", Строка.ТехнолГруппа);
