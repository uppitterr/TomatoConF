...
Процедура 	ЗаписатьДанныеНаСервере()
	Для каждого СтрокаДанных из Объект.ДанныеФайла  Цикл 
		НовыйОбъект = Справочники.ГОССОРТ.СоздатьЭлемент();
		НовыйОбъект.Патент = СтрокаДанных.патент;
		НовыйОбъект.Кодд = СтрокаДанных.кодд;
		НовыйОбъект.название = СтрокаДанных.название;
		НовыйОбъект.год =  СтрокаДанных.Год;
		НовыйОбъект.регионДопуска = СтрокаДанных.РегионДопуска;
		НовыйОбъект.ОригинаторПатентообладатель = СтрокаДанных.ОригинаторПатентообладатель;
		НовыйОбъект.Категория =  СтрокаДанных.Категория;
		НовыйОбъект.НаправлениеИспользования = СтрокаДанных.НаправлениеИспользования;
		НовыйОбъект.ПериодПотребления = СтрокаДанных.ПериодПотребления;
		НовыйОбъект.СрокСозревания = СтрокаДанных.СрокСозревания;
		НовыйОбъект.ТипРастения = СтрокаДанных.ТипРастения;
		НовыйОбъект.УсловияВыращивания = СтрокаДанных.УсловияВыращивания;
		НовыйОбъект.Форма = СтрокаДанных.Форма;
